...
/**
 * Memcached is designed to store all the latest clusters generated
 * sequentially for latest 24 hours.<br/>
...
public class MemcachedService {
  ...
  private static final ArrayDequeTranscoder ARRAY_DEQUE_LONG_TRANSCODER = new
    ArrayDequeTranscoder();
  private static final ClustersTranscoder CLUSTERS_TRANSCODER = new
    ClustersTranscoder();
  ...
  /**
   * @return list of timestamps which are stored in cache and which we have
   * data for.
   */
  ...
  public long addTimestamp() throws Exception {
    ...
    CASMutation<ArrayDeque<Long>> mutation = current -> {
      ...
      // first.
      ArrayDeque<Long> copy = new ArrayDeque<>(current);

      // Remove from the end (oldest) of list all which are older then
      // required time span
    ...
    // the key.
    ArrayDeque<Long> initialValue = new ArrayDeque<>();
    initialValue.addFirst(timestamp);

    // The mutator who'll do all the low-level stuff.
    CASMutator<ArrayDeque<Long>> mutator = new CASMutator<>(
      this.memcached,
      ARRAY_DEQUE_LONG_TRANSCODER
    );
  ...
  /**
   * @param group clusters group to be stored in memcached, timestamp - to be
   *              generated
   */
  ...
  public long saveClustersGroup(long timestamp, Clusters group)
  throws Exception {
    // This is how we modify a list when we find one in the cache.
    CASMutation<Clusters> mutation = current -> current;

    // The mutator who'll do all the low-level stuff.
    CASMutator<Clusters> mutator = new CASMutator<>(
      this.memcached,
      CLUSTERS_TRANSCODER
    );
  ...
  /**
   * @param delta period in millis to merge clusters for (since current
   *              timestamp)
   */
  public List<TweetsCluster> mergeFromTimestamp(long delta) {
    ArrayDeque<Long> timestamps = memcached.get(
      TIMESTAMPS_KEY,
      ARRAY_DEQUE_LONG_TRANSCODER
    );
    long now = new Date().getTime();
    LinkedHashMap<TweetsCluster, Integer> merged = new LinkedHashMap<>(
      64,
      0.75f,
      false
    );
